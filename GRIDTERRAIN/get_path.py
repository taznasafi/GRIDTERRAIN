...
class pathFinder:
    ...
    def get_path_for_all_feature_from_gdb(self, type ="Polygon", data_type="FeatureClass"):
        gdbPath =[]
        for dirpath, dirnames, filenames in arcpy.da.Walk(self.env_0, datatype=data_type, type=type):
            for filename in filenames:
                gdbPath.append(os.path.join(dirpath, filename))
        return gdbPath

    def get_file_path_with_wildcard_from_gdb(self, wildcard=None, data_type=None):
        arcpy.env.workspace = self.env_0
        if data_type =='Table':
            fc = arcpy.ListTables(wildcard)
        elif data_type == "raster":
            print("getting raster fc")
            fc = arcpy.ListRasters(wildcard,raster_type="All")
        else:
    ...
    def filter_List_of_featureclass_paths_with_wildcard(cls, path_link_list, wildcard):
        ...
        match = [x for x in path_link_list if fnmatch.fnmatch(os.path.basename(x), wildcard)]
        #print("found {} file(s)!!".format(len(match)))
        return match
